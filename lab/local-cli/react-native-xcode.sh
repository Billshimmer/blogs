# 基于react-native 0.42修改
# 1. 修改ENTRY_FILE 为由外部传入
# 2. 修改REACT_NATIVE_DIR 获取位置
# 3. 调用packager时增加--lab-extra参数
# 4. 在Debug 模式下不执行bundle
# 5. REACT_NATIVE_DIR改为固定值 node_modules/react-native
# 6. debug 模拟器也要设置https
...
if [[ "$CONFIGURATION" = "Debug" ]]; then
  echo "Skipping Debug CONFIGURATION"
  exit 0;
fi
