...
export default class Collapsible extends Component {
  ...
  constructor(props, context) {
    super(props, context);
    this._initHeight = this.props.initHeight ? this.props.initHeight : ALMOST_ZERO;
    this.state = {
      height: new Animated.Value(this._initHeight),
      contentHeight: 0,
      animating: false,
    };
  }
  ...
  _toggleCollapsed(collapsed) {
    const height = collapsed ? this._initHeight : this.state.contentHeight;
  ...
  _handleLayoutChange(event) {
    ...
    const height = this.props.collapsed ? this._initHeight : contentHeight;
    this.setState({
  ...
  render() {
    ...
    return (
      <Animated.View style={style} pointerEvents={this.props.collapsed ? 'box-none' : 'auto'}>
