...
var EventHandlers = {
  ...
  swipeStart: function (e) {
    e = e.nativeEvent; //use responder event
  ...
  swipeMove: function (e) {
    ...
    ////
    if (this._captureState == -1) {
      return;
    }
    if (this._captureState == 0) {
      var _dx = Math.abs(touchObject.curX - touchObject.startX);
      var _dy = Math.abs(touchObject.curY - touchObject.startY);
      if (_dy > 0.8 * _dx && _dy > 2) {
        this._captureState = -1;
      }
      if (0.8 * _dx > _dy && _dx > 8) {
        this._captureState = 1;
      }
      if (this._captureState != 1) {
        return;
