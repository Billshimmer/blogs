...
@property (nonatomic, copy  ) LaunchAdClickBlock   adClickBlock;    //广告点击
@property (nonatomic, assign) SkipShowType         showSkipType;
@property (nonatomic,strong ) CAShapeLayer         *shapelayer;
@end

@implementation LABLaunchAd
...
#pragma mark - 动画跳过
- (void)layoutSubviews {
  if(_showSkipType == SkipShowTypeAnimation) {
...
- (void)dispath_Tiemr {
  ...
  dispatch_source_set_event_handler(_dispatchTimer, ^{
    dispatch_async(dispatch_get_main_queue(), ^{
      NSString *title;
      if (_showSkipType == SkipShowTypeDefault) {
        long showTime = duration >= 1 ? duration : 1;
        title = [NSString stringWithFormat:@"%ld 跳过", showTime];
      } else {
        title = @"跳过";
      }
...
#pragma mark - 移除广告
- (void)launchAdRemove {
  [UIView animateWithDuration:_adDuration animations:^{
...
+ (instancetype)initImageWithAttribute:(NSInteger)adDuration showSkipType:(SkipShowType)showSkipType setLaunchAd:(SetLaunchAdBlock)setLaunchAd{
  static LABLaunchAd *launchAd = nil;
...
- (void)setWebImageWithURL:(NSString *)strURL options:(WebImageOptions)options result:(WebImageCompletionBlock)result adClickBlock:(LaunchAdClickBlock)adClickBlock {
  ...
  [_launchAdImgView setImageWithURL:[NSURL URLWithString:strURL] placeholderImage:kPlaceholderImage options:options completed:result?result:nil];
}

#pragma mark - 把本地的launchIamge作为广告图加载
- (void)setLocalImage {
  [self addSubview:self.launchAdImgView];
  _adClickBlock = [_adClickBlock copy];
  [_launchImgView setLocalImage:[self getLaunchImage]];
}
...
+ (void)clearDiskCache {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    NSString *path = [WebImageDownloader cacheImagePath];
    [fileManager removeItemAtPath:path error:nil];
    [WebImageDownloader checkDirectory:path];
  });
}
...
- (UIButton *)btnSkip {
  if(!_btnSkip) {
    ...
    if (_showSkipType == SkipShowTypeDefault) {
      _btnSkip.frame = CGRectMake(kScreen_Width-70, 30, 60, 30);
      ...
      _btnSkip.frame = CGRectMake(kScreen_Width-50, 30, 30, 30);
      _btnSkip.titleLabel.font = [UIFont systemFontOfSize:12];
