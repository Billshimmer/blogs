...
+ (NSString *)cacheImagePath {
  NSString *path =[NSHomeDirectory() stringByAppendingPathComponent:@"Library/LABLaunchAdCache"];
...
+(UIImage *)getCacheImageWithURL:(NSURL *)url {
  ...
  return [UIImage gifWithData:[NSData dataWithContentsOfFile:path]];
}
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholderImage completed:(WebImageCompletionBlock)completedBlock {
  [self setImageWithURL:url placeholderImage:placeholderImage options:WebImageDefault completed:completedBlock];
}
...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholderImage options:(WebImageOptions)options completed:(WebImageCompletionBlock)completedBlock {
  ...
  if (!options) options = WebImageDefault;
  //只加载,不缓存
  if (options & WebImageOnlyLoad) {
  ...
  UIImage *image = [WebImageDownloader getCacheImageWithURL:url];
  if (image && completedBlock) {
    weakSelf.image = image;
    if (image && completedBlock) completedBlock(image, url);
    if (options & WebImageDefault) return;
  }
  //先读缓存,再加载刷新图片和缓存
  [self dispatch_async:url result:^(UIImage *image, NSURL *url, NSData *data) {
    weakSelf.image = image;
    if (image && completedBlock) completedBlock(image, url);
    [WebImageDownloader saveImage:data imageURL:url];
  }];
}
...
#pragma mark - 异步加载图片
- (void)dispatch_async:(NSURL *)url result:(Dispatch_asyncBlock)result {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    NSData *data = [NSData dataWithContentsOfURL:url];
    UIImage *image = [UIImage gifWithData:data];
