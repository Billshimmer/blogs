...
@interface LABHighLightButton : UIButton

@property (nonatomic, strong) UIColor *highlightedColor;

@end

@implementation LABHighLightButton
...
@implementation LABAlertAction

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(LABAlertAction *action))handler {
  LABAlertAction *instance = [LABAlertAction new];
...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
  ...
  LABAlertController *instance = [LABAlertController new];
...
- (void)creatAllButtons {
  ...
  for (int i=0; i<self.actions.count; i++) {
    ...
    LABHighLightButton *btn = [LABHighLightButton new];
    btn.tag = 10+i;
...
- (void)didClickButton:(UIButton *)sender {
  LABAlertAction *action = self.actions[sender.tag-10];
...
- (NSArray<LABAlertAction *> *)actions {
