...
NSString *const MESSAGE_RECEIVED = @"LAB_PUSH_MESSAGE_RECEIVED";
NSString *const NOTIFICATION_RECEIVED = @"LAB_PUSH_NOTIFICATION_RECEIVED";

@interface LABPushManager() <JPUSHRegisterDelegate>

@end

@implementation LABPushManager

RCT_EXPORT_MODULE(LABPushModule);
...
+ (id)allocWithZone:(NSZone *)zone {
  static LABPushManager *sharedInstance = nil;
...
// 使用userdefault 来存储推送状态，
// key 为 "RemoteNotifications"
// value "registered" 表示已注册远程通知， value "unregistered" 表示未注册远程通知

RCT_EXPORT_METHOD(stopPush) {
  [[NSUserDefaults standardUserDefaults] setObject:@"unregistered" forKey:@"RemoteNotifications"];
...
RCT_EXPORT_METHOD(resumePush) {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:@"registered" forKey:@"RemoteNotifications"];
  [[NSUserDefaults standardUserDefaults] synchronize];
...
// 执行 unregisterForRemoteNotifications 之后，currentUserNotificationSettings 仍会保持开启推送前的状态

RCT_EXPORT_METHOD(getPushState:(RCTResponseSenderBlock)callback) {
  
  NSInteger currentType = 0;
  NSString *config = [[NSUserDefaults standardUserDefaults] objectForKey:@"RemoteNotifications"];
  
  // 如果config为空，说明用户还没有执行过推送开关，这个时候推送一定是开启状态
  if (config) {
    if ([config isEqualToString:@"registered"]) {
      currentType = 1;
    } else if ([config isEqualToString:@"unregistered"]) {
      currentType = 0;
    }
  } else {
    currentType = 1;
  ...
  callback(@[[NSNull null], [NSNumber numberWithInteger:currentType]]);
}
...
- (void)networkDidLogin:(NSNotification *)notification {
  ...
  [JPUSHService registrationIDCompletionHandler:^(int resCode, NSString *registrationID) {
    if (resCode == 0) {
      NSLog(@"registrationID get success: %@", registrationID);
    } else {
      NSLog(@"registrationID get failed");
    }
  }];
}

// 两个字典合并，dic2合并到dic1
- (NSDictionary *)dictionaryCombineWithDic1:(NSDictionary *)dic1 Dic2:(NSDictionary *)dic2 {
  NSMutableDictionary *parameters = [[NSMutableDictionary alloc] initWithDictionary:dic1];
  [parameters addEntriesFromDictionary:dic2];
  return parameters;
}
...
- (void)networkDidReceiveMessage:(NSNotification *)notification {
  ...
  NSDictionary *dic = [self dictionaryCombineWithDic1:userinfo Dic2:extras];
  
  [self.bridge.eventDispatcher sendAppEventWithName:MESSAGE_RECEIVED
                                               body:extras];
}

- (void)receiveRemoteNotification:(NSNotification *)notification {
  NSDictionary *obj = [notification object];
  [self.bridge.eventDispatcher sendAppEventWithName:NOTIFICATION_RECEIVED body:obj];
}
...
RCT_EXPORT_METHOD( setTags:(NSArray *)tags
  ...
  [JPUSHService setTags:tagSet alias:nil fetchCompletionHandle:^(int iResCode, NSSet *iTags, NSString *iAlias) {
    callback(@[iResCode == 0 ? @"" : @"error", @(iResCode)]);
  }];
}
...
RCT_EXPORT_METHOD( removeAlias:(RCTResponseSenderBlock)callback) {
  self.asyCallback = callback;
