...
//        showImageView(thumbPath, imageView, filePath, message);
    ...
    private void loadImage(String thumbPath) {
        if (new File(thumbPath).exists()){
            Glide.with(context).load(thumbPath)
                    .override(DisplayUtils.dp2px(100),DisplayUtils.dp2px(100))
                    .fitCenter()
                    .placeholder(R.drawable.ease_default_image)
                    .into(imageView);
        }else if (new File(imgBody.getLocalUrl()).exists()){
            Glide.with(context).load(imgBody.getLocalUrl())
                    .override(DisplayUtils.dp2px(100),DisplayUtils.dp2px(100))
                    .fitCenter()
                    .placeholder(R.drawable.ease_default_image)
                    .into(imageView);
        }else{
            GlideUrl glideUrl = new GlideUrl(imgBody.getThumbnailUrl()+"?thumbnail", new LazyHeaders.Builder()
                    .addHeader("share-secret", ((EMImageMessageBody) message.getBody()).getThumbnailSecret())
                    .addHeader("Authorization", "Bearer " + EMClient.getInstance().getAccessToken())
