...
export type State = {
  serverData: AttributeMap; //服务端数据，save成功之后才会更新
  pendingOps: Array<OpsMap>; //等待提交的操作队列 一旦执行save会push一个，使得在执行save期间提交的修改不会影响正在提交的，之后如果成功会pop失败会合并，一般情况队列长度为一
  objectCache: ObjectCache; //Object的 Object字段
  tasks: TaskQueue; //保证对同一个对象的操作队列执行
  existed: boolean
};
