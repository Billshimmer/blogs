...
export default class ParseObject {
  ...
  get attributes(): AttributeMap {
    ...
    //返回serverData与所有pendingOps合并之后的数据
  ...
  _getSaveParams(): SaveParams {
    ...
    if (this.className === '_User') {
      path = 'users'; //User特殊处理 修改使User的path始终为users
    }
    if (this.id) {
      path += '/' + this.id;
    }
  ...
  _migrateId(serverId: string) {
    if (this._localId && serverId) {
      if (singleInstance) {
        ...
        if (oldState) {
          //将在objectState中 localId对应的数据转移到 serverId下
  ...
  _handleSaveError() {
    var pending = this._getPendingOps();
    let stateController = CoreManager.getObjectStateController();
    //save 失败 将本次未成功的PendingOps 与下一个合并
  ...
  fetch(options: RequestOptions): ParsePromise {
    ...
    return ParsePromise._thenRunCallbacks(controller.fetch(this, true, fetchOptions), options);
  }
  ...
  save(
    ...
    return ParsePromise._thenRunCallbacks(controller.save(unsaved, saveOptions).then(() => {
  ...
  destroy(options: RequestOptions): ParsePromise {
    ...
    return ParsePromise._thenRunCallbacks(CoreManager.getObjectController().destroy(
      this,
      destroyOptions
    ), options);
  }
  ...
  static fetchAll(list: Array<ParseObject>, options) {
    ...
    return ParsePromise._thenRunCallbacks(CoreManager.getObjectController().fetch(
      list,
      true,
      queryOptions
    ), options);
  ...
  static saveAll(list: Array<ParseObject>, options) {
    ...
    return ParsePromise._thenRunCallbacks(CoreManager.getObjectController().save(
      list,
      saveOptions
    ), options);
  }
