...
import labAxios from 'lab4/core/axios';
...
labAxios.setLogResponseInterceptor(ajax, 'RESTController');
...
const RESTController = {
  ...
  ajax(method: string, url: string, data: any, headers?: any) {
    ...
    if (__DEV__) {
      console.log('RESTController ajax method:' + method + ' url:' + url + ' data:', data, 'headers:', headers);
    }
    ...
    var dispatch = function() {
      ...
      xhr.onreadystatechange = function() {
        ...
        if (__DEV__) {
          console.log('RESTController ajax onreadystatechange url:' + url + ' xhr:', xhr);
        }
      ...
      // version TODO
      headers['lab-param'] = 'version=' + (global.__version__ || '1.0.0');
  ...
  */
  ...
  request(method: string, path: string, data: mixed, options?: RequestOptions) {
    ...
    // NOTE parse统一使用 POST 通过在payload._method设置真实的method，其服务端两种方式都支持
    if (method !== 'POST') {
    ...
    }).then(null, function(resError) {
      // Transform the error into an instance of ParseError by trying to parse
      // the error string as JSON
      var error;
      if (resError && resError.response) {
        if (resError.response.data) {
          error = new ParseError(resError.response.data.code, resError.response.data.message);
          // 修改 增加isErrorResponse 标记表示是服务器返回的错误
          error.isErrorResponse = true;
        } else {
          error = new ParseError(
            ParseError.INVALID_JSON,
            'Received an error with invalid JSON'
          );
        }
      } else {
        error = new ParseError(
          ParseError.CONNECTION_FAILED,
          'XMLHttpRequest failed: ' + resError.message
        );
      }
