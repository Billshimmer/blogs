...
export default class ParseQuery {
  ...
  get(objectId: string, options?: FullOptions): ParsePromise {
    ...
    return ParsePromise._thenRunCallbacks(this.first(firstOptions).then((response) => {
      if (response) {
        return response;
      }
  ...
  find(options?: FullOptions): ParsePromise {
    ...
    return ParsePromise._thenRunCallbacks(controller.find(
      this.className,
      this.toJSON(),
      findOptions
    ).then((response) => {
  ...
  each(callback: (obj: ParseObject) => any, options?: BatchOptions): ParsePromise {
    ...
    if (this._order || this._skip || (this._limit >= 0)) {
      ...
      return ParsePromise._thenRunCallbacks(ParsePromise.error(error), options);
    }
    ...
    var finished = false;
    return ParsePromise._thenRunCallbacks(ParsePromise._continueWhile(() => {
      return !finished;
    }, () => {
